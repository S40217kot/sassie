...
export const feelingChoices = ["\u9759\u304b", "\u4e00\u606f\u3064\u304f", "\u305d\u3063\u3068\u706f\u3059", "\u5927\u4e08\u592b\u3060\u3088", "\u5fc3\u306b\u7559\u3081\u308b"];

function rewriteTrace(count) {
  if (count === 0) return "\u8a00\u8449\u3092\u66f8\u304d\u76f4\u3055\u305a\u306b\u3001\u624b\u653e\u3057\u307e\u3057\u305f";
  if (count === 1) return "\u4e00\u5ea6\u3001\u8a00\u8449\u3092\u66f8\u304d\u76f4\u3057\u307e\u3057\u305f";
  return `${count}\u56de\u3001\u8a00\u8449\u3092\u66f8\u304d\u76f4\u3057\u307e\u3057\u305f`;
}

function polishLegacyTrace(trace) {
  const match = /^\u5165\u529b\u5185\u5bb9\u3092\u3059\u3079\u3066\u6d88\u3057\u305f\u56de\u6570\uff1a(\d+)\u56de$/.exec(trace);
  return match ? rewriteTrace(Number(match[1])) : trace;
}

function traceMetadata(traces) {
  let writingDuration = 1;
  let rewriteCount = 0;

  for (const trace of traces) {
    const duration = /\u9001\u4fe1\u307e\u3067(?:(\d+)\u5206)?(\d+)\u79d2\u304b\u304b\u308a\u307e\u3057\u305f/.exec(trace);
    if (duration) writingDuration = Number(duration[1] || 0) * 60 + Number(duration[2]);

    const numberedRewrite = /^(\d+)\u56de(?:\u3001)?\u8a00\u8449\u3092\u66f8\u304d\u76f4\u3057\u307e\u3057\u305f$/.exec(trace)
      ?? /^(\d+)\u56de\u66f8\u304d\u76f4\u3057\u307e\u3057\u305f$/.exec(trace)
      ?? /^\u5165\u529b\u5185\u5bb9\u3092\u3059\u3079\u3066\u6d88\u3057\u305f\u56de\u6570\uff1a(\d+)\u56de$/.exec(trace);
    if (numberedRewrite) rewriteCount = Number(numberedRewrite[1]);
    if (trace === "\u4e00\u5ea6\u3001\u8a00\u8449\u3092\u66f8\u304d\u76f4\u3057\u307e\u3057\u305f" || trace === "\u4e00\u5ea6\u3059\u3079\u3066\u6d88\u3057\u307e\u3057\u305f") {
      rewriteCount = 1;
    }
  }

  return { writingDuration, rewriteCount };
}

function parsePost(row) {
  const traces = JSON.parse(row.traces_json).map(polishLegacyTrace);
  const detailTraces = JSON.parse(row.detail_traces_json).map(polishLegacyTrace);
  const metadata = traceMetadata(detailTraces);
  return {
    id: row.id,
    userId: row.user_id,
    ...
    traces,
    detailTraces,
    writingDuration: metadata.writingDuration,
    rewriteCount: metadata.rewriteCount,
...
export async function createPost({ length, duration, clearedCount, userId }) {
  ...
  const clearedText = rewriteTrace(clearedCount);
  const post = {
    id: randomUUID(),
    userId,
    ...
    length,
    traces: [clearedText, durationText],
    detailTraces: [clearedText, durationText],
    writingDuration: duration,
    rewriteCount: clearedCount,
