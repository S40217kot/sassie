...
export async function registerUser(nickname, password) {
  ...
  try {
    ...
    if (message.includes("UNIQUE") || message.includes("nickname_reserved")) return null;
    throw error;
  }
...
export async function authenticateUser(nickname, password) {
  ...
  `, [nickname]);
  if (!await passwordMatchesUser(user, password)) return null;
...
export async function updateUserNickname(userId, nickname) {
  try {
    ...
    const message = String(error);
    if (message.includes("nickname_rate_limit")) return { status: "rate-limited" };
    if (message.includes("UNIQUE") || message.includes("nickname_reserved")) {
...
export async function deleteOwnAccount(userId, password) {
  const user = await db.get(`
    SELECT id, password_hash, password_salt, is_system, is_super_admin
    FROM users
    WHERE id = ?
  `, [userId]);
  if (!user) return "not-found";
  if (user.is_system || user.is_super_admin) return "protected";
  if (!await passwordMatchesUser(user, password)) return "invalid-password";
  ...
  await db.batch([
    ...
    { sql: "DELETE FROM nickname_change_events WHERE user_id = ?", args: [userId] },
