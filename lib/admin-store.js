...
export async function updateManagedNickname(targetId, nickname, actor) {
  const target = await db.get(`
    SELECT id, is_system, is_admin, is_super_admin FROM users WHERE id = ?
  `, [targetId]);
  if (!target) return "not-found";
  ...
  if (!actor.isSuperAdmin && target.is_admin) return "protected";

  try {
    await db.run("UPDATE users SET nickname = ? WHERE id = ?", [nickname, targetId]);
  } catch (error) {
    const message = String(error);
    if (message.includes("nickname_rate_limit")) return "rate-limited";
    if (message.includes("UNIQUE") || message.includes("nickname_reserved")) return "unavailable";
    throw error;
  }
  return "ok";
}
