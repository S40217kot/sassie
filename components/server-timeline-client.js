...
export function ServerTimelineClient() {
  const [posts, setPosts] = useState([]);

  useEffect(() => {
    fetch("/api/posts").then((response) => response.json()).then((data) => setPosts(data.posts ?? []));
  }, []);
  ...
  return (
    <main className="phone">
      ...
      <section className="timeline" aria-label="タイムライン">
        {posts.map((post) => <TraceCard post={post} key={post.id} />)}
      </section>
